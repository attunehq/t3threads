...
export async function fixture(t: TestContext, handler?: (req: IncomingMessage, res: ServerResponse) => boolean | undefined) {
  ...
  const server = createServer((req, res) => {
    ...
    if (url.pathname === "/.well-known/t3/environment") return json(res, descriptor);
    ...
    if (["/api/orchestration/shell", "/api/orchestration/snapshot"].includes(url.pathname)) return json(res, { projects: [project], threads: [...stored.values()] });
  ...
  const target: Target = { name: "local", home: dir, origin, descriptor, config: { command: [process.execPath, authCli] } };
  ...
  return { dir, authLog, configPath, target, api: new Api(target, "test-secret"), commands, stored, control,
